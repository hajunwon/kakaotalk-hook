...
KakaoTalk LOCO 프로토콜 리버스 엔지니어링 기반의 Frida hooking 도구 및 TypeScript 클라��언트 라이브러리.

## 주요 기���

- **Frida Hooking** — KakaoTalk 앱의 HTTP/LOCO 통신 모니터링, 안티 디텍션 우회, 디바이스 스푸핑
- **LOCO 프���토콜 클라이언트** — TypeScript로 구현된 KakaoTalk LOCO 프로토콜 (인증, 메시징, 세션 관리)
...
└── src/                    # TypeScript 클라이언트 라이브러리
    ...
    ├── application/        # 커맨드 ��스패처, 이벤트 버스, 푸시 핸들러
...
# 또는 ��접 config.json 작성 (config.example.json 참고)
