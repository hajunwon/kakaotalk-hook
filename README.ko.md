...
- **ADB 디바이스 관리** — 디바이스 연결, frida-server 배포, APK/XAPK/Split APK 설치 자동화
...
│   └── xapk-installer.js   # XAPK/APK/Split APK 설치
├── frida/                  # Frida 스크립트
...
1. 연결된 디바이스 목록 확인
2. APK/XAPK/Split APK 설치 (단일 `.apk`, `.xapk`/`.apks` 번들, 또는 `base.apk` + `split_config.*.apk`가 들어 있는 디렉토리)
3. frida-server 배포
